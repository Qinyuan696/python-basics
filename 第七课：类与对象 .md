...
注意：如果上面的程序去掉：`people.__init__(self, n, a, w)`，即不继承父类的函数，则输出：`说: 我 0 岁了，我在读 3 年级`，因为子类的构造方法把父类的构造方法覆盖了。
...
![https://img-blog.csdnimg.cn/20191007090316462.png](https://img-blog.csdnimg.cn/20191007090316462.png)
...
创建实例对象的时候不应该包含在类中；类中的每个方法都必须至少有一个参数self，且必须是方法的第一个形参，self参数代表当前对象。
...
class Ticket():
    ...
    def __init__(self,a,c,d):
        ...
        self.date = d
    def compute(self):
        if self.date=='weekend':
            self.date=1.2
        if self.date=='workday':
            self.date=1
        all_price=(self.adult+self.child*0.5)*self.date*self.price
        return all_price
    
tt=Ticket(2,1,'workday')
print(tt.compute())

tt=Ticket(2,1,'weekend')
print(tt.compute())

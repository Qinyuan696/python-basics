...
### 4.6 `dict.setdefault(key, default=None)`
...
- `key in dict` `in` 操作符用于判断**键`key`**是否存在于字典中，如果键在字典 dict 里返回`true`，否则返回`false`。而`not in`操作符刚好相反，如果键在字典 dict 里返回`false`，否则返回`true`。
